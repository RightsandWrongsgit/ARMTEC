...
## General Philosophy
All products to be (distributed under the Bristol-Myers Company name (or the name of one of its divisions) should be thoroughly tested prior to distribution in the marketplace. This applies to new products and to revised formulas of existing products (i.e., changes in ingredients, color, fragrance, etc.).
Periodic retesting of established formulas is also recommended, particularly vs new competitive introductions and revised/improved competitive formulas, so that Bristol-Myers' products are maintained as high quality entries in their respective categories. Specifically, no products sold under the Bristol-Myers name or that of one of its divisions should be marketed unless there is evidence that it is at least as acceptable to consumers as are competitive entries with which it is intended to compete.
The difficulty associated with product testing OTC's is recognized. When practical, however, organoleptic characteristics should be tested.

## Standards of Comparison
Consumers' stated attitudes are not an absolute indicator of their subsequent marketplace behavior. For this reason a control for relative evaluation should be built into the design of every product test. The standard of comparison or control selected is, of course, dependent on the purpose of the test. For example. . . .
1. If introducing a new product, the standard of comparison should be the anticipated competitive target such as the leading competitive brand in the category or category segment in which the product will compete.
2. If introducing a new formula of an existing product, evaluation would minimally be vs the current formula. Others, of course, would be included if considered beneficial to the product evaluation.
Results of a given product test are sometimes compared to results of prior tests in the same category to put them in some perspective. This is frequently done with monadic test results or monadic sections of paired comparisons. The utilization of this "normative" data as the control or standard of comparison must be done with great caution. Markets change, competitive formulas change, consumer attitudes change. Prior tests should be used as the control only if the previous test was done in the recent past and the tests were conducted under comparable conditions. It is not practical to specify a time limit since a category with little or no change (no new entries, no revised price differentials, no new positionings, etc.) can tolerate longer time intervals than one where consumers are continuously bombarded with new introductions, revised ingredients, etc. The safest position is to build a control into every test that will serve as the standard of comparison for that test. Comparisons with prior tests can then be made as a matter of interest with recognition of the limitations involved.
There may be situations when there is no appropriate product on the market to test against. It should be noted, however, that conducting any type of product test without a benchmark or control against which to read the results is an extremely risky proposition given the difficulty one will experience in trying to interpret the results from such a test. One way to avoid this problem, if a concept for the product exists, is to conduct a simulated test market concept/product test and use the volume estimate as the criterion.

## Marketplace Significance
A basic assumption behind all product testing, regardless of the methodology, is that repeat purchase is directly related to the consumer's reaction to the product under conditions of normal usage, thus, results will bear some relationship to that aspect of marketplace reality. However, the multiple marketplace variables that influence consumer choice and purchasing, i.e., trial, distribution, quantity usage, etc. preclude the use of product test results as predictors of in-market relative sales volumes. Consumer input via product testing provides meaningful guidance for improving products. As long as we subsequently evaluate product diagnostics, we are probably on the path to producing products with greater consumer acceptance and, if the other elements of the marketing mix are working adequately, with greater sales and profits. Hence, while not precisely predictive of volume, consumer product testing can provide information to help management better understand the magnitude of likely opportunities and risks.
